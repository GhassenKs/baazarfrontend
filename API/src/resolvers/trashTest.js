...
const [createList, { data:newList }] = useMutation(CREATE_LIST);
...
const CREATE_LIST= gql`
  mutation createList($userId: String!) {
   createList(ListInput:{user:$userId}){items{title},id}
...
const FIND_ORDER = gql`
    query findList($id:String) {findList(id:$id){id,user{firstName},items {
                _id
                id
                title
                description
